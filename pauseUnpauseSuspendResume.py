...
def pauseUnpauseSuspendResume(auth_url, tenant, user, password, server_name, timePerAction=20):
	"""pause, unpause, suspend and resume a VM within a specified time for each operation"""
	...
	def pauseInstance(server):
		try:
			...
			server.pause()
			actionGood = False
			while not actionGood and time.time() - action_start < timePerAction:
